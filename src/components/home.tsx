...
const Home = ({
  ...
  return (
    <div>
      <p className={"font-medium px-7 text-xs mt-[32px] mb-[20px] text-left"}>
      ...
      {languages?.map(({ icon, name, value }) => {
        return (
          ...
          >
            <div
              className={
                "flex justify-between px-[12px] mb-[24px] items-center"
              }
            >
              ...
              {active === value ? (
                ...
                <div
                  className={
                    "w-4 h-4 p-2 rounded-full border border-[#D7D6D5] border-dashed"
                  }
                />
              )}
            </div>
      ...
      <div className={"w-full px-7 "}>
        <button
          ...
          disabled={!active}
          className={
            "focus:outline-none focus:ring-0 bg-[#100F0E] rounded-[20px] text-xs text-white  mt-[24px] w-full py-8 "
          }
        >
          {loading ? (
            <CircleLoader
